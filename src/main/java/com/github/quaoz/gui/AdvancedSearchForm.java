...
public class AdvancedSearchForm {
	...
	public AdvancedSearchForm() {
		...
		searchButton.addActionListener(e -> {
			...
			String location = Geocoder.standardise(locationField.getText().strip());

			Double sizeLower = null;
			Double sizeUpper = null;

			try {
				sizeLower = Double.parseDouble((String) sizeLowerSpinner.getValue());
			} catch (NumberFormatException ignored) {
			}

			try {
				sizeUpper = Double.parseDouble((String) sizeUpperSpinner.getValue());
			} catch (NumberFormatException ignored) {
			}
			...
			if (sizeLower != null) {
				records = MothManager.collectMoths(String.format("%s:%s", sizeLower, sizeUpper), 2, 100, (s1, s2) -> {
