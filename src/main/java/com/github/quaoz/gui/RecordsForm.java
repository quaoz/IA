...
public class RecordsForm {
	...
	public RecordsForm() {
		records = RecordManager.getSpecies(Main.getGui().getRecord().getName());
		$$$setupUI$$$();
	...
	private void createUIComponents() {
		...
		String[] columnNames = {"ID", "Species", "Location", "Date", "Size", "User"};
		String[][] data = new String[records.size()][6];

		int count = 0;
		for (Record record : records) {
			data[count][0] = String.valueOf(record.getId());
			data[count][1] = record.getSpecies();
			data[count][2] = record.getLocation();
			data[count][3] = record.getDate();
			data[count][4] = String.valueOf(record.getSize());
			data[count][5] = record.getUser();
			count++;
		}
